...
namespace FormsGenerator
{
    class Program
    {
        ...
        static List<Type> GetTypes(string input)
        {
            ...
            return assembly.GetTypes().Where(type => type.IsClass && !type.IsAbstract).ToList();
        }

        static void GenerateForms(List<Type> types)
        {
            ...
            var htmlFields = new Dictionary<Type, string>()
            {
                { typeof(Frase), fieldHtmlTemplate.Replace("VAR_INPUT_TYPE", "<input type=\"text\" id=\"VAR_NAME\" name=\"VAR_NAME\">") },
                { typeof(Texto), fieldHtmlTemplate.Replace("VAR_INPUT_TYPE", "<textarea id=\"VAR_NAME\" name=\"VAR_NAME\" rows=\"4\" cols=\"50\"></textarea>") },
                { typeof(Data), fieldHtmlTemplate.Replace("VAR_INPUT_TYPE", "<input type=\"date\" id=\"VAR_NAME\" name=\"VAR_NAME\">") }
            };

            foreach (var type in types)
            {
                var fields = type.GetFields().Where(field => field.FieldType.IsSubclassOf(typeof(Campo))).ToList();
                var formFields = "";
                ...
                foreach (var field in fields)
                {
                    ...
                    var showName = ((Campo)field.GetValue(null)).Verboso ?? inputName.Replace("_", " ").TitleCase();
                ...
                var formHtml = formHtmlTemplate.Replace("VAR_FIELDS", formFields);
                System.IO.File.WriteAllText($"{type.Name.ToLower()}.html", formHtml);
            }
        }
        ...
        static void GenerateDatabaseAccess(List<Type> types)
        {
            ...
            var fieldSqlTypes = new Dictionary<Type, string>()
            {
                { typeof(Frase), "TEXT" },
                { typeof(Texto), "TEXT" },
                { typeof(Data), "TEXT" }
